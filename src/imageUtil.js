...
const cropArtworkFromCard = (cardImage) => {
  ...
  const artWorkHeight = 0.42;

  return cardImage
    .clone()
    .crop(
      cardImage.bitmap.width * horizontalOffset,
      cardImage.bitmap.height * verticalOffset,
...
const cropPocketFromScan = (scanImage, slotIndex) => {
  ...
  const pocketWidth =
    Math.floor(config.input.cropArea.width / config.input.cols) -
    config.input.colSpacing / 2;
  const pocketHeight =
    Math.floor(config.input.cropArea.height / config.input.rows) -
    config.input.rowSpacing / 2;
  const pocketOffset = {
...
const combineVertically = (images) =>
  new global.Promise((resolve) => {
...
const gallerize = (images) =>
  new global.Promise((resolve) => {
    ...
    images = images.map((image) =>
      image.scaleToFit(maxImageWidth, maxImageHeight, Jimp.RESIZE_BICUBIC)
    );

    // calculate totalt width/height used
    // in order to center align gallery
    const firstRowTotalWidth = _sum(
      images.slice(0, cols).map((image) => image.bitmap.width)
    );
    const firstRowMaxHeight = Math.max(
      ...images.slice(0, cols).map((image) => image.bitmap.height)
    );
